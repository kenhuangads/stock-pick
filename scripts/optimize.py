...
def compute_weights(stats, opt_cfg, prev_doc=None):
    ...
    for s in STRATEGIES:
        ...
        if st["trades"] >= opt_cfg["min_trades"] and st["expectancy"] is not None:
            ...
            if exp_frac < opt_cfg["disable_expectancy_below"]:
                enabled = False
                w = 0.0
            else:
        ...
        st["enabled"] = enabled
        weights[sid] = w
        was = prev_enabled.get(sid)
        if was is not None and was != enabled:
            log.append(f"策略「{s['name']}」{'停用（期望值轉負，汰弱）' if not enabled else '重新啟用（績效回升，留強）'}")
...
def run_optimize(reviews, cfg, prev_doc, as_of_date):
    ...
    doc = {
        ...
        "stats": stats,
        "log": log[-200:],
        "meta": {s["id"]: {"name": s["name"], "desc": s["desc"]} for s in STRATEGIES},
    }
    return weights, doc
